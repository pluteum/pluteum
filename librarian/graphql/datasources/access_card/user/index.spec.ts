...
describe("User", () => {
  ...
  it.todo(
    "#login will throw an AuthenticationError if the user does not exist"
  );
  it.todo("#login will throw an AuthenticationError with invalid credentials");
  it.todo(
    "#login will throw an AuthenticationError if no default library can be found"
  );
  it.todo(
    "#login will return the user, library, an access token and a refreshToken"
  );
  it.todo(
    "#login will not include the users hashed password, password reset token or refresh token"
  );
  ...
  it.todo(
    "#reset will throw an AuthencationError if the reset token (JWT) has expired"
  );
  it.todo(
    "#reset will throw an ResourceNotFoundError if a user cannot be found"
  );
